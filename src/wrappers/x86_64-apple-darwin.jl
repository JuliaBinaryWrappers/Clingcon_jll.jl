# Autogenerated wrapper script for Clingcon_jll for x86_64-apple-darwin
export clingcon, libclingcon

using Clingo_jll
JLLWrappers.@generate_wrapper_header("Clingcon")
JLLWrappers.@declare_executable_product(clingcon)
JLLWrappers.@declare_library_product(libclingcon, "@rpath/libclingcon.dylib")
function __init__()
    JLLWrappers.@generate_init_header(Clingo_jll)
    JLLWrappers.@init_executable_product(
        clingcon,
        "bin/clingcon",
    )

    JLLWrappers.@init_library_product(
        libclingcon,
        "lib/libclingcon.dylib",
        RTLD_LAZY | RTLD_DEEPBIND,
    )

    JLLWrappers.@generate_init_footer()
end  # __init__()
